...
from util.helper_functions import mkdir
...
def save_run_configs_to_yaml(
        default_config: Dict, run_configs: Union, run_names: List[str], path: str = ""
):
    for run_config, name in zip(run_configs, run_names):
        save_run_config_to_yaml(default_config, name, path, run_config)


def save_run_config_to_yaml(default_config, name, path, run_config):
...
def extract_dicts_from_options(
        raw_config: Dict,
) -> (Union[List[dict], List[Union[dict, Any]]], List[dict]):
    run_configs = [dict()]
    run_configs_for_run_names = [dict()]
    identifier_list = []
    ...
    for settings_key, settings in raw_config.items():
        if not isinstance(settings, dict):
            ...
            run_configs, run_configs_for_run_names = extract_run_configs_from_list(
                run_configs, run_configs_for_run_names, settings, settings_key, identifier_list
            )
        else:
